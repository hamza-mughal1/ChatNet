...
class PostsModel:
    ...
    def create_dict(obj, request):
        from models.users_model import (
            UsersModel,
        )  # import inside a function due to circlular import error
    ...
    async def create_post(self, db, title, content, file, token_data, request):
        if file:
            if file.content_type.split("/")[1] not in self.allowed_post_image_type:
                raise HTTPException(
                    status_code=400,
                    detail=f"only {self.allowed_post_image_type} types are allowed",
                )
            ...
            if len(img) > self.image_size * 1024 * 1024:
                raise HTTPException(
                    status_code=400,
                    detail=f"Request body size exceeds {self.image_size}MB limit.",
                )
        ...
        post = DbPostModel(
            user_id=token_data["user_id"], title=title, content=content, image=url
        )
    ...
    def delete_post(self, db: utils.db_dependency, id, token_data, request):
        ...
        if not post.user_id == token_data["user_id"]:
            raise HTTPException(
                status_code=403, detail="Access denied. (Don't have ownership)"
            )
    ...
    def like_post(self, db: utils.db_dependency, post_id, token_data, request, rds):
        if (
            post := db.query(DbPostModel).filter(DbPostModel.id == post_id).first()
        ) is None:
            raise HTTPException(status_code=404, detail="post not found")
        ...
        if (
            ...
            return PostsModel.get_post(
                self, db, post_id, request, rds, update_cache=True
            )
