...
class CommentsModel:
    ...
    def comment_on_post(self, db: utils.db_dependency, id, comment, token_data):
        ...
        new_comment = Comments(
            user_id=token_data["user_id"], post_id=id, content=comment.content
        )
        db.add(new_comment)
        post.comments += 1
        db.commit()
    ...
    def get_comment_by_post_id(self, db: utils.db_dependency, id, page, rds):
        ...
        for i in (
            ...
            if user_name is None:
                user_name = (
                    db.query(Users).filter(Users.id == i.user_id).first().user_name
                )
    ...
    def get_by_comment_id(self, db: utils.db_dependency, id):
        ...
        user_name = (
            db.query(Users).filter(Users.id == comment.user_id).first().user_name
        )
        comment = utils.orm_to_dict(comment)
