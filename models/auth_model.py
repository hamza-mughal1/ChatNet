...
def login(
    ...
    limit: Annotated[ApiLimitDependency, Depends(login_api_limit)],
    user_credentials: OAuth2PasswordRequestForm = Depends(),
):
    user = (
        db.query(db_models.Users)
        .filter(db_models.Users.email == user_credentials.username)
        .first()
    )
    if not user:
    ...
    access_token = Oauth2.create_token(
        {"user_id": user.id, "user_name": user.user_name, "type": "access-token"}
    )
...
def refresh(
    db: utils.db_dependency,
    token: str = Depends(Oauth2.oauth2_baerer),
    refresh_token: Annotated[str | None, Header()] = None,
):
    if refresh_token is None:
        raise HTTPException(
            status_code=403, detail="unable to find 'Refresh-token' header"
        )
    ...
    if (
        refresh_db_model := db.query(db_models.RefreshTokens)
        .filter(db_models.RefreshTokens.token == refresh_token)
        .first()
    ) is None:
        raise HTTPException(
            status_code=403,
    ...
    if (
        access_db_model := db.query(db_models.AccessTokens)
        .filter(db_models.AccessTokens.id == refresh_db_model.access_token_id)
        .first()
    ).token != token:
        raise HTTPException(
            status_code=403,
    ...
    user = (
        db.query(db_models.Users)
        .filter(db_models.Users.id == refresh_db_model.user_id)
        .first()
    )
...
def logout_user(
    ...
    if (
        db.query(db_models.AccessTokens)
        .filter(db_models.AccessTokens.id == refresh_db_model.access_token_id)
        .first()
    ).token != token:
        raise HTTPException(
    ...
    acc_tokens = (
        db.query(db_models.AccessTokens)
        .filter(db_models.AccessTokens.user_id == refresh_db_model.user_id)
        .all()
    )
    for i in acc_tokens:
        db.delete(i)
