...
def test_update_user(client):
    ...
    response = client.put(
        "users/",
        json={
            "name": UserData.name.value + "test",
            "bio": "testbio",
            "user_name": UserData.user_name.value + "test",
            "email": "test" + UserData.email.value,
        },
        headers=header,
    )
    assert response.status_code == 200
...
def test_logout_user(client):
    ...
    tokens = login_user(client)
    header_logout = {
        "Authorization": "Bearer " + tokens.json().get("access_token"),
        "Refresh-token": tokens.json().get("refresh_token"),
    }
    header = {"Authorization": "Bearer " + tokens.json().get("access_token")}
    response = client.put(
        "users/",
...
def test_change_password(client):
    ...
    response = client.patch(
        ...
        json={
            ...
            "password": UserData.password.value,
            "new_password": UserData.password.value + "1",
        },
        headers=header,
    )
    ...
    assert (
        client.post(
            ...
            data={
                ...
                "password": UserData.password.value + "1",
            },
        ).status_code
        == 200
    )
...
def test_image_upload(client):
    ...
    with open("test_image.png", "rb") as f:
        ...
        response = client.post(
            "users/upload-profile-picture/", files=files, headers=header
        )
        assert response.status_code == 200
